...
/** Named points on a shape that a dimension's endpoint can bind to, and that the Move tool offers
 * as pickable anchors. Rotation-aware for rects. Guides and dimensions have no bindable points. */
function listBindablePoints(
  shape: TShape,
): { key: TShapePointKey; point: { x: number; y: number } }[] {
  switch (shape.type) {
    case 'line':
    ...
    case 'rect': {
      const origin = { x: shape.x, y: shape.y };
      const corners: { key: TShapePointKey; local: { x: number; y: number } }[] = [
